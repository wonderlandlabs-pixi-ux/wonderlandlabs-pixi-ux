...
describe('StyleTree', () => {
  ...
  describe('Basic operations', () => {
    ...
    it('should expand object values with setMany recursively by default', () => {
      tree.setMany('button', [], {
        background: {
          color: 'red',
          alpha: 0.8,
      ...
      expect(tree.get('button.background.color', [])).toBe('red');
      expect(tree.get('button.background.alpha', [])).toBe(0.8);
      expect(tree.get('button.border.color', [])).toBe('blue');
    });

    it('should only expand one level when setMany recursion is disabled', () => {
      const background = {
        color: 'red',
        alpha: 0.8,
      };
      ...
      expect(tree.get('button.background', [])).toEqual(background);
      expect(tree.get('button.background.color', [])).toBeUndefined();
      expect(tree.get('button.border', [])).toBe('blue');
    });
  ...
  describe('Wildcard matching', () => {
    ...
    it('should match wildcard paths that return object values', () => {
      const palette = {
        background: { color: 'rgb(222, 232, 255)' },
        border: { color: 'rgb(48, 71, 118)' },
      };
      tree.set('*.logo', [], palette);
  ...
  describe('Edge cases', () => {
    ...
    it('should preserve nested object values during wildcard matches', () => {
      const complexValue = {
        background: { color: '#dfe8ff' },
        border: { color: '#304776' },
      };
      tree.set('*.sidebar', [], complexValue);

      expect(tree.match({
        nouns: ['root', 'sidebar'],
        states: [],
      })).toEqual(complexValue);
    });
