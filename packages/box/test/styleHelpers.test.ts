...
describe('styleHelpers', () => {
  it('falls back to the local cell path for wildcard style lookups', () => {
    ...
    styles.set('*.background', [], { fill: 'rgb(244, 246, 251)' });
    ...
    expect(resolveStyleValue([styles], logo, ['background'])).toEqual({ fill: 'rgb(244, 246, 251)' });
  ...
  it('reads nested properties from object-backed style entries', () => {
    const styles = new StyleTree();
    styles.set('*.logo', [], {
      background: { fill: 'rgb(222, 232, 255)' },
    ...
    expect(resolveStyleValue([styles], logo, ['background'])).toEqual({ fill: 'rgb(222, 232, 255)' });
