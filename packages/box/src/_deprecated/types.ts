import {z} from 'zod';
import {ALIGN, MEASUREMENT_ENUM_CANONICAL, MEASUREMENT_ENUM_INPUT, UNIT_BASIS,} from './constants.js';
...
const PercentMeasurementObjectSchema = z.object({
  mode: z.literal(UNIT_BASIS.PCT),
  value: z.number().finite().refine((value) => value >= 0 && value <= 1, {
    message: '% value must be between 0 and 1',
  }),
});

const StarMeasurementObjectSchema = z.object({
  mode: z.literal(UNIT_BASIS.STAR),
  value: z.number().finite().refine((value) => value >= 0, {
